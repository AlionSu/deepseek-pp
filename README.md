...
1.12.2 是工具执行透明化与生产体验修复版本：工具失败不再黑箱、可一键导出诊断日志、长文产出自动保存、复制完整输出、导出内容范围可选，并让内联智能体的续跑计数与实际步数一致。
...
| 长文产出 | Agent 产出长文时自动保存为产物并提供下载入口，无需再手动要求调用下载工具。 |
...
<details>
<summary>展开历史版本变更回顾（1.12.1 - 0.2.0）</summary>
